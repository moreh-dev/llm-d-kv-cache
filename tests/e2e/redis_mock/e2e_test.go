...
func (s *KVCacheSuite) TestCacheHit() {
	...
	pods, err := s.indexer.GetPodScores(s.ctx, nil, prompt, nil, defaultModelName, fakePodList)
	s.Require().NoError(err)
...
func (s *KVCacheSuite) TestPrefixReduction() {
	...
	pods, err := s.indexer.GetPodScores(s.ctx, nil, fullPrompt, nil, defaultModelName, []string{s.Pod1IP})
	s.Require().NoError(err)
	s.T().Logf("Received pod scores: %+v", pods)
	...
	pods, err = s.indexer.GetPodScores(s.ctx, nil, midPrompt, nil, defaultModelName, []string{s.Pod1IP})
	s.Require().NoError(err)
	...
	pods, err = s.indexer.GetPodScores(s.ctx, nil, shortPrompt, nil, defaultModelName, []string{s.Pod1IP})
	s.Require().NoError(err)
...
func (s *KVCacheSuite) TestPrefixExpansion() {
	...
	// Test 1: short prompt
	pods, err := s.indexer.GetPodScores(s.ctx, nil, shortPrompt, nil, modelName, []string{s.Pod1IP})
	s.Require().NoError(err)
	s.T().Logf("Received pod scores: %+v", pods)
	s.Empty(pods, "expected no pod scores")
	...
	// Test 2: mid prompt
	pods, err = s.indexer.GetPodScores(s.ctx, nil, midPrompt, nil, modelName, []string{s.Pod1IP})
	s.Require().NoError(err)
	...
	// Test 3: full prompt
	pods, err = s.indexer.GetPodScores(s.ctx, nil, fullPrompt, nil, modelName, []string{s.Pod1IP})
	s.Require().NoError(err)
...
func (s *KVCacheSuite) TestLongPrefixExpansion() {
	...
	pods, err = s.indexer.GetPodScores(s.ctx, nil, longPrompt, nil, modelName, []string{s.Pod1IP})
	s.Require().NoError(err)
...
func (s *KVCacheSuite) TestChatCompletionsE2E() {
	...
	pods, err := s.indexer.GetPodScores(s.ctx, nil, flattenedPrompt, nil, "ibm-granite/granite-3.3-8b-instruct", []string{s.Pod1IP})
	s.Require().NoError(err)
...
func (s *KVCacheSuite) TestCacheHitWithLocalTokenizer() {
	...
	pods, err := s.indexer.GetPodScores(s.ctx, nil, prompt, nil, modelName, fakePodList)
	s.Require().NoError(err)
