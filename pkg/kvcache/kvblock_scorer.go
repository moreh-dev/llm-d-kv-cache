...
//
// ScoringStrategy defaults to LongestPrefixMatch as a moreh-dev safe baseline:
// HMA bitmask indexing remains active (since ModelConfigs are populated and the
// Pool tags events with StoredGroups), but the HybridPrefixCacheScorer is not
// auto-selected. Operators who want HMA-aware scoring can opt in by clearing
// ScoringStrategy (auto-detect) or setting it to HybridPrefixMatch.
